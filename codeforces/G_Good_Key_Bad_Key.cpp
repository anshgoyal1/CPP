...
ll help(vi a, ll n, ll pos, ll bad, ll k)
{
    if (pos == n)
    {
        return 0;
    ...
    ll ans = num - k + help(a, n, pos + 1, bad, k);

    ans = max(ans, num / 2 + help(a, n, pos + 1, bad + 1, k));
    return dp[pos][bad] = ans;
}
void solve()
{
    ll n, k;
