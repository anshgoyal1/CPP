...
Node *surroundings(Node *root,int k,Node** parent){
    if(root->data = k){
        return root;
    }else if(root->data > k){
        *parent = root;
        root->left = surroundings(root->left,k,parent);
    }else if(root->data < k){
        *parent = root;
        root->right = surroundings(root->right,k,parent);
    }
}
